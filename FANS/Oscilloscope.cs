...
namespace FANS
{
    public partial class Oscilloscope : Form
    {
        ...
        private void _initDAC(object sender=null, EventArgs e=null)
        {
            ...
            AI_Channels.Instance.PointsPerBlock=Convert.ToInt32(PointsPerBlock.Text);
            AI_Channels.Instance.ACQ_Rate=Convert.ToInt32(Acqusition_Rate.Text);
            this.ReloadChannels();
        }
        ...
        private void _initFileSystem()
        {
            ...
            {
                TimeTraceFileManager.Instance.workfolder = FolderName.Text;
                if (FolderName.Text == "") FolderName.Text = TimeTraceFileManager.Instance.workfolder;
                while (TimeTraceFileManager.Instance.FileExists(filename_textbox.Text))
                {
                    filename_textbox.Text = TimeTraceFileManager.Instance.suggestFileNameWithIncrement(filename_textbox.Text);
                }


                if (RawDataRecording.Checked)
                {
                    if (timeLimit == 0) AllCustomEvents.Instance.OnMeasurementStatusChanged(this, new StatusEventArgs("Recording", 0, 0, 0));
                    TimeTraceFileManager.Instance.PrepareForRAW_RealTimeDataRecording(filename_textbox.Text, timeLimit);
                }
                else TimeTraceFileManager.Instance.PrepareForRealTimeDataRecording(filename_textbox.Text, ImportantConstants.Rload, ImportantConstants.K_Ampl_first_Channel, AI_Channels.Instance.ACQ_Rate, timeLimit);
            }
        ...
        {
            channel_4_DAC_Enabled.Checked = false;
            channel_3_DAC_Enabled.Checked = false;
            channel_2_DAC_Enabled.Checked = false;
            channel_1_DAC_Enabled.Checked = true;

            _initDAC();

            _initGraphics();
            _initFileSystem();
            MeasurementThread.Instance.StartThread(TimeTraceManager.ContiniousAcquisitionWithPresiseVoltageMeasurement);
        ...
        private void ReloadChannels()
        {
         ...
         #region Change of Different Parameters of Oscilloscope
